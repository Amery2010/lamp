 ...
 * 校验省份数字编号的正则
 */
const regProvince = /^[1-9][0-9]$/

/**
 * 校验生日日期的正则
 */
const regBirthday = /^((18|19|20)\d{2})(\d{2})(\d{2})$/

/**
 * 校验 18 位身份证的正则
 */
const regIDNumber = /^[1-9]\d{5}(18|19|20)\d{9}[0-9Xx]$/

/**
 * 各省份的数字编号表
 */
const provinceNumber: ProvinceNumber = {11: '北京', 12: '天津', 13: '河北', 14: '山西', 15: '内蒙古', 21: '辽宁', 22: '吉林', 23: '黑龙江',
 ...
 * @returns 是否为现有的省份编号
 */
function checkProvince (num: string): boolean {
 ...
 * @returns 生日日期是否符合规范
 */
function checkBirthday (date: string): boolean {
  if (regBirthday.test(date)) {
    // 将生日字符串替换为可以被 Date 格式化的日期形式
    const birthday: string = date.replace(regBirthday, '$1-$3-$4')
    // 将日期转换为 ISO 格式，如 2019-10-11T00:00:00.000Z
    try {
      const ISODate: string = new Date(birthday).toISOString()
 ...
 * @returns 校验码是否正确
 */
function checkCode (idNum: string): boolean {
  // 加权因子表
  const factor: Array<number> = [ 7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2 ]
  // X与校验码换算表
  const parity: Array<number | string> = [ 1, 0, 'X', 9, 8, 7, 6, 5, 4, 3, 2 ]
 ...
 * @since v0.1.0
 */
function checkIDNumber (idNum: string, province?: string): boolean {
  const provinceStr: string = idNum.substring(0, 2)
  const birthdayStr: string = idNum.substring(6, 14)
  if (province) {
    // 如果传入了省份限定值，则优先校验省份
    let provinceId = ''
    // 遍历获取省份名称对应的编号
    for (const idx in provinceNumber) {
      if (provinceNumber[idx] === province) provinceId = idx
    }
